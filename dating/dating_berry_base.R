...
collage_data <- read.csv('C:\\Users\\Sandeep Kumar\\Google Drive\\Ed Research\\Heterogenous treatment effects\\git_code\\hte\\dating\\dating_data.csv')
str(collage_data)

#changing some variable to factors
col_names <- c("male","orientation","asian","mideast",
...
collage_data <- na.omit(collage_data)
#Making a copy
collage_data2 <- collage_data

TreatmentVar <- 'manipulation'
PredictorsVar <- c("age","male","orientation","asian",
                   "vote_score_10","u_acct_level", "view_sent_cnt_1",
                   "view_rcvd_cnt_1","msg_sent_cnt_1","msg_rcvd_cnt_1",
                   "match_sent_cnt_1","match_rcvd_cnt_1")
OutcomeVar <-  "match_rcvd_cnt_2"

#Dividing control and treatments groups so as to fit seperate models
control_data <- collage_data %>% filter(manipulation == 0) 
treatment_data <- collage_data %>% filter(manipulation == 1) 
#-------------------------------------------------------------------------------------------------------------------
...
RFModel_c <- train(match_rcvd_cnt_2 ~ age+male+orientation+asian+mideast+black+natam+indian+pacisl+latin+white+other+vote_score_10+u_acct_level+view_sent_cnt_1+view_rcvd_cnt_1+msg_sent_cnt_1+msg_rcvd_cnt_1+match_sent_cnt_1+match_rcvd_cnt_1,
                   control_data,
...
model <- rpart(match_rcvd_cnt_2 ~ age+orientation+asian+mideast+black+natam+indian+pacisl+latin+white+other+vote_score_10,
      control_data)

#Training a Random Forest model on treatment group data
RFModel_t <- train(view_rcvd_cnt_1 ~ age+male+orientation+asian+mideast+black+natam+indian+pacisl+latin+white+other+vote_score_10+u_acct_level+view_sent_cnt_1+view_rcvd_cnt_1+msg_sent_cnt_1+msg_rcvd_cnt_1+match_sent_cnt_1+match_rcvd_cnt_1,
                   treatment_data,
                   method = "rf",
                   metric = "RMSE",
                   tuneLength = 3,
